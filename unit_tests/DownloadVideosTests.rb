...
class DownloadVideosTests < Test::Unit::TestCase
    ...
    def test_download_videos_with_id_with_accept_language
        stub_request(:post, "https://api.gettyimages.com/v3/downloads/videos/123").with(headers: {"Accept-Language" => "de"}).with(query: {"auto_download" => "false"})
            .to_return(body: '{ "message": "success" }')
